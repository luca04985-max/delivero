-- ========================================
-- DELIVERO DATABASE COMPLETE SETUP
-- ========================================
-- Questo script ricrea tutte le tabelle necessarie
-- e inserisce dati di test completi
...
CREATE TABLE IF NOT EXISTS order_items (
    ...
    customizations TEXT[], -- Array di personalizzazioni
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE IF NOT EXISTS rider_locations (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(order_id) -- Solo una location per ordine
);
...
('demo.customer@delivero.local', '$2b$10$rQZ8ZkQGQmQJQJQJQJQJQu12345678901234567890123456789012345678901', 'Mario Rossi', 'customer', '+39 333 1234567', 'Via Roma 123, 00100 Roma'),
('demo.customer2@delivero.local', '$2b$10$rQZ8ZkQGQmQJQJQJQJQJQu12345678901234567890123456789012345678901', 'Laura Bianchi', 'customer', '+39 333 2345678', 'Via Milano 456, 00100 Roma'),
('demo.rider@delivero.local', '$2b$10$rQZ8ZkQGQmQJQJQJQJQJQu12345678901234567890123456789012345678901', 'Paolo Verdi', 'rider', '+39 333 3456789', 'Via Torino 789, 00100 Roma'),
('demo.rider2@delivero.local', '$2b$10$rQZ8ZkQGQmQJQJQJQJQJQu12345678901234567890123456789012345678901', 'Giulia Neri', 'rider', '+39 333 4567890', 'Via Napoli 321, 00100 Roma'),
('demo.manager@delivero.local', '$2b$10$rQZ8ZkQGQmQJQJQJQJQJQu12345678901234567890123456789012345678901', 'Admin User', 'manager', '+39 333 5678901', 'Via Firenze 654, 00100 Roma'),
('admin@delivero.local', '$2b$10$rQZ8ZkQGQmQJQJQJQJQJQu12345678901234567890123456789012345678901', 'Super Admin', 'admin', '+39 333 6789012', 'Via Venezia 987, 00100 Roma')
...
(1, 5, 1, 2.50), -- Coca Cola
(1, 7, 1, 5.00), -- Tiramisù
(2, 9, 2, 8.00), -- Uramaki California
(2, 10, 1, 4.50), -- Nigiri Salmone
(2, 12, 1, 3.50), -- Miso Soup
(3, 13, 1, 7.50), -- Classic Burger
(3, 17, 1, 3.00), -- Patatine Fritte
(4, 2, 1, 10.00), -- Diavola
(4, 6, 2, 1.50)  -- Acqua Naturale
ON CONFLICT DO NOTHING;
...
-- Rider Locations (per ordini attivi)
INSERT INTO rider_locations (order_id, rider_id, latitude, longitude, eta_minutes) VALUES
(2, 3, 41.8985, 12.5114, 10), -- Ordine #2, rider Paolo
(3, 3, 41.9028, 12.4964, 15), -- Ordine #3, rider Paolo
(5, 4, 41.8850, 12.5322, 20)  -- Ordine #5, rider Giulia
ON CONFLICT DO NOTHING;
...
-- View per ordini attivi con tracking
CREATE OR REPLACE VIEW active_orders_with_tracking AS
SELECT 
    o.*,
    ...
    rl.eta_minutes,
    rl.timestamp as last_location_update
FROM orders o
LEFT JOIN users u ON o.customer_id = u.id
LEFT JOIN restaurants r ON o.restaurant_id = r.id
LEFT JOIN users rider ON o.rider_id = rider.id
LEFT JOIN rider_locations rl ON o.id = rl.order_id
WHERE o.status IN ('accepted', 'preparing', 'delivering');

-- View per statistiche admin
...
SELECT 
    ...
    (SELECT COUNT(*) FROM orders WHERE status = 'delivered' AND DATE(created_at) = CURRENT_DATE) as today_delivered,
...
RETURNS DECIMAL AS $$
DECLARE
    R DECIMAL := 6371; -- Raggio Terra in km
    dLat DECIMAL;
    dLon DECIMAL;
    a DECIMAL;
    c DECIMAL;
...
-- Funzione per cleanup vecchie location (24 ore)
...
-- Mostra conteggio record inseriti (SAFE VERSION)
DO $$
DECLARE
    table_name TEXT;
...
BEGIN
    ...
    LOOP
        BEGIN
            ...
            RAISE NOTICE '%: % records', table_name, COALESCE(record_count, 0);
        EXCEPTION WHEN OTHERS THEN
            RAISE NOTICE '%: Table exists but query failed', table_name;
        END;
    END LOOP;
