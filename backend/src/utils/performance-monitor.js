...
class PerformanceMonitor {
  ...
  logMetrics() {
    ...
    const latestLoad = this.metrics.system.loadAverage[this.metrics.system.loadAverage.length - 1] || 0;
    ...
    console.log(`   DB: ${this.metrics.database.queryCount} queries | ${this.metrics.database.avgResponseTime.toFixed(1)}ms avg`);
    console.log(`   Requests: ${this.metrics.application.requests} | Errors: ${this.metrics.application.errors}`);
    console.log('');
  }
  ...
  logSummary() {
    const uptime = process.uptime();
    const avgCpu = this.metrics.system.cpu.reduce((a, b) => a + b, 0) / this.metrics.system.cpu.length;
    const avgMem = this.metrics.system.memory.reduce((a, b) => a + b, 0) / this.metrics.system.memory.length;
    const avgLoad = this.metrics.system.loadAverage.reduce((a, b) => a + b, 0) / this.metrics.system.loadAverage.length;
    ...
    console.log(`   Error Rate: ${this.metrics.application.errors > 0 ? (this.metrics.application.errors / this.metrics.application.requests * 100).toFixed(2) : 0}%`);
    console.log('');
  }
